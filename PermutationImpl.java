	...
	public class PermutationImpl implements Permutation {
		 ...
		 private static void checkForDuplicatesInList(List<Integer> list) throws RuntimeException {
		    ...
		    private static void checkForElementsOutOfRange(List<Integer> list, int size) throws RuntimeException {
		        for (int i : list) {
		            if (i < 1 || i > size) {
		                throw new RuntimeException();
		            }
		...
		public Set<List<Integer>> getAllCycles(){
			...
			return listToSet(getAllCycles_(new ArrayList<List<Integer>>(), elementsMap, 1));
		}
